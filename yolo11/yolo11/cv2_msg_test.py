...
class ImageTools(object):
    ...
    def convert_ros_msg_to_ros_compressed_msg(self, image, image_encoding='bgr8', compressed_format="jpg"):
    ...
    def convert_to_cv2(self, image):
        ...
        if type(image) == np.ndarray:
            cv2_img = image
        elif type(image) == type(Image()):
            cv2_img = self.convert_ros_msg_to_cv2(image)
        elif type(image) == type(CompressedImage()):
...
if __name__ == '__main__':
    ...
    #print("convert_ros_msg_to_cv2:")
    #cv2_img = it.convert_ros_msg_to_cv2(img)
    cv2_img = cv2.imread('lenna.jpg')
    print(type(cv2_img))
